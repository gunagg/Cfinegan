...
from model_bag import G_NET, D_NET
...
class FineGAN_evaluator(object):
	...
	def __init__(self):
		...
		self.save_dir = os.path.join(cfg.SAVE_DIR, 'images_cartoons')
	...
	def evaluate_finegan(self):
		if cfg.TRAIN.NET_G == '':
			...
			for p_cl in range(cfg.SUPER_CATEGORIES) :
				for c_cl in range(cfg.FINE_GRAINED_CATEGORIES) :
					parent_class = p_cl 
					child_class = c_cl
					...
					for k in range(10) :	
						...
						self.save_image(fake_imgs[2][0], self.save_dir, 'child_final'+str(p_cl) + "_" + str(c_cl)+"_noise_"+str(k))
					print("%d steps reached"%(cfg.FINE_GRAINED_CATEGORIES * p_cl + c_cl))
	...
	def save_image(self, images, save_dir, iname):
		...
		vutils.save_image(
			images.data, full_path, normalize=True)
